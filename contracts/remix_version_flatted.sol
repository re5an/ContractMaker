...
contract contractMaker is Ownable, Freezable {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view returns (bool upkeepNeeded, bytes memory performData) {
        ...
        // address[] memory _todayContracts;
        if(_todayContractIDs.length > 0){
            // for(uint i=0; i < _todayContractIDs.length; i++){
            //     _todayContracts[i] = address(contracts[i]);
            // }
            upkeepNeeded = true;
            performData = abi.encode(_todayContractIDs);
            // performData = abi.encode(_todayContracts);
        } else {
            upkeepNeeded = false;
        }
    ...
    function performUpkeep(bytes calldata performData) external {
        // address[] memory _todayContracts = abi.decode(performData, (address[]));
